...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	*/
	int parent = 0;
	int indexPosition = dySize(heap);

	dyAdd(heap, value); /* adds to end now need to adjust position */

	while (indexPosition != 0) {
		parent = (indexPosition - 1) / 2;  //Floor implied through use of int
		if (compare(dyGet(heap, parent), dyGet(heap, indexPosition)) == 1) {

		dySwap(heap, parent, indexPosition);

			indexPosition = parent;
		}
		else
			return;
	}
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
	...
	//store last elepment into first and the dec size before adjusting heap
